...
void TestNetworking() {
	...
	bool canConnect = client -> Connect(127, 0, 0, 1, port);
		...
		if (TutorialGame::fileExists("score1.txt")) {
			int score1;
			...
			score1 = sVal;
			infile.close();
			server->SendGlobalPacket(StringPacket(std::to_string(score1)));
		}
		if (TutorialGame::fileExists("score2.txt")) {
			int score2;
			...
			score2 = sVal;
			infile.close();
			server->SendGlobalPacket(StringPacket(std::to_string(score2)));
		}
		if (TutorialGame::fileExists("score3.txt")) {
			int score3;
			...
			score3 = sVal;
			infile.close();
			server->SendGlobalPacket(StringPacket(std::to_string(score3)));
		}
